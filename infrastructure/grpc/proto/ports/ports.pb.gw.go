...
/*
Package ports is a reverse proxy.
...
// RegisterCreatePortsHandlerServer registers the http handlers for service CreatePorts to "mux".
// UnaryRPC     :call CreatePortsServer directly.
...
// RegisterUpdatePortsHandlerServer registers the http handlers for service UpdatePorts to "mux".
// UnaryRPC     :call UpdatePortsServer directly.
// StreamingRPC :currently unsupported pending https://github.com/grpc/grpc-go/issues/906.
...
// RegisterCreatePortsHandlerFromEndpoint is same as RegisterCreatePortsHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterCreatePortsHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterCreatePortsHandler(ctx, mux, conn)
}

// RegisterCreatePortsHandler registers the http handlers for service CreatePorts to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterCreatePortsHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterCreatePortsHandlerClient(ctx, mux, NewCreatePortsClient(conn))
}

// RegisterCreatePortsHandlerClient registers the http handlers for service CreatePorts
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "CreatePortsClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "CreatePortsClient"
...
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterUpdatePortsHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	conn, err := grpc.Dial(endpoint, opts...)
	if err != nil {
		return err
	}
	...
	return RegisterUpdatePortsHandler(ctx, mux, conn)
}

// RegisterUpdatePortsHandler registers the http handlers for service UpdatePorts to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterUpdatePortsHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterUpdatePortsHandlerClient(ctx, mux, NewUpdatePortsClient(conn))
}

// RegisterUpdatePortsHandlerClient registers the http handlers for service UpdatePorts
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "UpdatePortsClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "UpdatePortsClient"
// doesn't go through the normal gRPC flow (creating a gRPC client etc.) then it will be up to the passed in
